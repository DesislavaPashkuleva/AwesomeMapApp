...
final class LoginViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        usernameTextField.setPlaceholderColor(.white)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showCongratsSegue" {
            if let navigationController = segue.destination as? UINavigationController,
                let congratsViewController = navigationController.topViewController as? CongratsViewController {
                    congratsViewController.username = usernameTextField.text
                    congratsViewController.password = passwordTextField.text
            }
        }
    }
